...
class RegressionDataMaker:
    ...
    def make_data(self):
        ...
        y = X @ coefs[1:] + coefs[0] + np.random.normal(0, self.noise, size=(self.n_samples, 1)).reshape(-1,1)
        return X, y, coefs
    
    # save the generated data to a csv file
    def save_data(self, X, y, filename):
        np.savetxt(filename, np.hstack((X, y.reshape(-1,1))), delimiter=',')
        print(f'Data saved to {filename}')
        
    # save the generated coefs to a csv file
    def save_coefs(self, coefs, filename):
        np.savetxt(filename, coefs, delimiter=',')
        print(f'Coefs saved to {filename}')
    ...
    # make a function to create regeression data with the first column as ones
